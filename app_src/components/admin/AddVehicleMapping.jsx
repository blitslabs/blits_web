...
// API
import { createVehicleMapping, getServiceVehicles, getServiceFarePlans, getZones } from '../../utils/api'

class AddVehicleMapping extends Component {
    ...
    componentDidMount = () => {
        document.title = "Crear Vinculación de Vehículo"
        const { token } = this.props
        let serviceVehicles, serviceFarePlans, zones

        serviceVehicles = getServiceVehicles({ token })
        serviceFarePlans = getServiceFarePlans({ token })
        zones = getZones({ token })

        Promise.all([serviceVehicles, serviceFarePlans, zones])
            .then(async ([serviceVehicles, serviceFarePlans, zones]) => {
    ...
    handleSubmitBtn = (e) => {
        ...
        createVehicleMapping({ serviceVehicleId, serviceFareId, zoneId, description, token })
            .then(data => data.json())
            .then((res) => {
                console.log(res)
                if (res.status == 'OK') {
                        ...
                        <div className="col-md-6 col-xs-12 col-sm-12 grid-margin stretch-card">
                            <div className="card">
                                <div className="card-body">
                                    <div className="card-body">
                                        <h6 className="card-title">Añadir Vinculación Vehículo - Tarifa - Zona</h6>
                                        <form className="forms-sample">
                                            ...
                                            <button onClick={this.handleSubmitBtn} className="btn btn-primary mr-2">Crear Vínculación</button>
...
export default connect(mapStateToProps)(AddVehicleMapping)
